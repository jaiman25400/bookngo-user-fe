...
const SkatesPage = () => {
  ...
  const [venues, setVenues] = useState<SkatingVenue[]>([]);
  const [filteredVenues, setFilteredVenues] = useState<SkatingVenue[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedVenue, setSelectedVenue] = useState<SkatingVenue | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const { data, error } = await fetchSkatingData();
  ...
  useEffect(() => {
    if (!mapContainer.current || !venues.length || map.current) return;
    ...
    const updateTooltipPosition = () => {
      if (hoveredVenueRef.current && map.current) {
        const p = map.current.project([
          hoveredVenueRef.current.longitude,
          hoveredVenueRef.current.latitude,
        ]);
    ...
    venues.forEach((venue) => {
      ...
      pin.className = "skate-marker-pin";
      pin.setAttribute("aria-hidden", "true");
      pin.setAttribute("aria-label", `${venue.name}, ${venue.city}`);
      pin.textContent = "⛸️";

      pin.addEventListener("mouseenter", () => {
        pin.classList.add("skate-marker-pin-hover");
        hoveredVenueRef.current = venue;
        setHoveredVenue(venue);
        if (map.current) {
          const p = map.current.project([venue.longitude, venue.latitude]);
          setTooltipPosition({ x: p.x, y: p.y });
        }
      });
      pin.addEventListener("mouseleave", () => {
        pin.classList.remove("skate-marker-pin-hover");
        hoveredVenueRef.current = null;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-500 text-sm mt-1">Finding rinks near you</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800 mb-2">Error Loading Rinks</h2>
  ...
  return (
    <div className="flex h-screen bg-slate-50 flex-col lg:flex-row overflow-hidden">
      ...
      <div className="w-full lg:w-[480px] xl:w-[540px] h-full flex flex-col bg-white border-r border-slate-200 shadow-xl flex-shrink-0">
        <div className="p-6 pb-5 border-b border-slate-100 bg-gradient-to-br from-sky-500 to-blue-600">
          ...
          <p className="text-white/90 mt-2 text-sm">
            {venues.length} rink{venues.length !== 1 ? "s" : ""} across Canada — hover skating icons for details, click to visit
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
          {filteredVenues.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[280px] p-8 text-center">
              ...
              <h3 className="text-xl font-semibold text-slate-800 mb-2">No rinks found</h3>
        ...
        {process.env.NEXT_PUBLIC_MAPBOX_TOKEN && mapLoaded && (
          <div className="absolute inset-0 pointer-events-none rounded-none lg:rounded-l-2xl">
            {hoveredVenue && tooltipPosition && (
              <div
                className="skate-marker-tooltip skate-marker-tooltip-visible absolute"
                style={{
                ...
                role="tooltip"
              >
                <span className="skate-marker-tooltip-title">{hoveredVenue.name}</span>
                <span className="skate-marker-tooltip-city">{hoveredVenue.city}</span>
                <span className="skate-marker-tooltip-hint">Click to view venue</span>
              </div>
            )}
          </div>
        )}
      ...
      <style jsx global>{`
        ...
        .skate-marker-tooltip-visible {
          opacity: 1;
          visibility: visible;
        }
        .skate-marker-tooltip-title {
          display: block;
          font-size: 14px;
          font-weight: 700;
          color: #0f172a;
        ...
        .skate-marker-tooltip-city {
          display: block;
          font-size: 12px;
          color: #64748b;
          margin-bottom: 6px;
        }
        .skate-marker-tooltip-hint {
          display: block;
          font-size: 10px;
          color: #0284c7;
          font-weight: 500;
